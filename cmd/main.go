...
func main() {
	...
	rootCmd.PersistentFlags().String(flags.FlagValidatorMnemonic, "pledge help butter initial squirrel roast middle chimney daring chat until biology horn feed focus write arctic promote junior pizza carbon timber captain achieve", "validator mnemonic")
	...
	rootCmd.PersistentFlags().String(flags.FlagValidatorMnemonic2, "level repeat wrap name unhappy daughter final opera dry inject enough system trap latin wisdom group time long boss true fly acid nephew cheap", "validator mnemonic 2")
