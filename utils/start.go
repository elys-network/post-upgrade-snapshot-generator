...
func (cw PromptWriter) Write(data []byte) (int, error) {
	...
	coloredData := []byte(cw.color + "[" + cw.moniker + "]" + types.ColorReset + " " + string(data))
...
func Start(cmdPath, homePath, rpc, p2p, pprof, api, moniker, successColor, errorColor string, additionalArgs ...string) *exec.Cmd {
	// Set the log level
	logLevel := "info"
